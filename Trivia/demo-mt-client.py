...
def convert_to_byte_string(num):
    """
    Convert a number to 4 bytes representation and reverse it.
    ...
    :type num: int
    :return: The reversed byte representation of the input number.
    :rtype: bytes
    """
...
def main():
    ...
    while True:
        try:
            ...
            # convert to binary
            data_bytes = (chr(message_type)).encode()

            len_of_data = len(json.dumps(message_data))
            data_bytes += convert_to_byte_string(len_of_data)

            data_bytes += json.dumps(message_data).encode()
